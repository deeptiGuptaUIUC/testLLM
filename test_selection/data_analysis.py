...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", type=str, required=True, default="microsoft/Phi-3-mini-4k-instruct")
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, device_map="auto", cache_dir='../cache', output_hidden_states=True, offload_folder="offload")
    ...
    for i in tqdm(range(len(sampled_data))):
        ...
        if args.prompt == 'wiz':
            ...
            if input_i == '':
                ...
                prompt_to_use = PROMPT_DICT["prompt_no_input"].format_map(temp_dict)
                whole_text = prompt_to_use + output_i
                instruct_i = prompt_to_use
            else:
                temp_dict = {'instruction': instruct_i, 'input': input_i}
                prompt_to_use = PROMPT_DICT["prompt_input"].format_map(temp_dict)
                whole_text = prompt_to_use + output_i
                instruct_i = prompt_to_use
